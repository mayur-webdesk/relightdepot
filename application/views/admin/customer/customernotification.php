		...
		<div class="page-content">
			<div class="row">
				<div class="col-md-12">
					...
					<h3 class="page-title">
						Volusion to BigCommerce Customer Reset Password
					</h3>
					<ul class="page-breadcrumb breadcrumb">
						<li>
							<i class="fa fa-home"></i>
							<a href="<?php echo $this->config->site_url();?>admin/customer/usernotification">
							...
							<tbody>
								<?php 
									...
									if(isset($customer_data) && !empty($customer_data) && count($customer_data)>0)
									{
										...
										{	
											?>
											<tr <?php if($no==1) { echo 'class="start_process"'; } ?>  data-code="<?php echo $d['customer_id'] ?>" data-column="<?php echo $no; ?>"  >
												<td><?php echo $no++ ;?></td>
...
function sendRquest()
{
	...
	var column=jQuery('.start_process').attr('data-column');
	if(code){
		jQuery('.processing').removeClass('processing');
		jQuery('.start_process').find('.respose_tag').html('Please wait...');
		jQuery('.start_process').addClass('processing');
		$.ajax({
			url: '<?php echo $this->config->site_url();?>/admin/customer/resetpasswrod',
			data: {
				code: code,
				column: column,
				send:'yes'
